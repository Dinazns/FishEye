...
function setMediaPart(name, media) {
    ...
    function setupLightbox() {
        ...
        function updateLightboxImage(sens,index) {
    ...
    setupLightbox();
    function updateLightboxImage(sens,i) {
        if (i !== null) {
            i = (sens === "right") ? (i + 1) % cards.length : (i - 1 + cards.length) % cards.length;
            document.querySelector('.lightBox img, .lightBox video').src = cards[i].src; // Met à jour la source de l'image ou de la vidéo dans la lightbox en utilisant l'index calculé
        } else {
            console.error("Position non trouvée");
        }
    }
        ...
        if (mediaElement.image) {
            ...
            img.addEventListener("click", () => {
                ...
                side_right.addEventListener('click', () => updateLightboxImage("right",index));
                side_left.addEventListener('click', () => updateLightboxImage("left",index));
            });
